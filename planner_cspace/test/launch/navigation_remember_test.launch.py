...
def generate_test_description():
    set_env = SetEnvironmentVariable("GCOV_PREFIX", "/tmp/gcov/planner_cspace_navigation_remember")
    gtest = Node(
        package="planner_cspace",
        executable="test_navigate_remember",
    ...
    launch_file = IncludeLaunchDescription(
        FrontendLaunchDescriptionSource(
            os.path.join(
                ...
                "test",
                "navigation_remember_rostest.test",
            )
        )
    )
