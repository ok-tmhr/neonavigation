...
def generate_test_description():
    set_env = SetEnvironmentVariable("GCOV_PREFIX", "/tmp/gcov/planner_cspace_planner_2dof_serial_joints")
    gtest = Node(
        package="planner_cspace",
        executable="test_planner_2dof_serial_joints",
    ...
    launch_file = IncludeLaunchDescription(
        XMLLaunchDescriptionSource(
            os.path.join(
                ...
                "test",
                "planner_2dof_serial_joints_rostest.test",
            )
        )
    )
